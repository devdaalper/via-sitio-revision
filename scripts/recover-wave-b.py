"""Recover the official wave B centerlines without modifying the PNG.
...
source=root/'public/assets/brand/wave-b.png'
a=np.array(Image.open(source).convert('RGBA'))
colors=[rgb for rgb,n in Counter(map(tuple,a[a[:,:,3]>160,:3])).most_common() if n>500]
# Each of the 43 original solid ink colors identifies a separate centerline.
paths=[]
...
import json
output=root/'src/data/wave-b-centerlines.json'
output.write_text(json.dumps({
 'source':'Ejemplo urvas 2.png — equipo de diseño, 2026-08-20',
 'viewBox':[0,0,1529,1507],
 'lines':paths,
